...
class AnimeSamaParser @Inject constructor() : AnimeParser {
    ...
    override suspend fun parseAnimeDetails(html: String): Anime {
        ...
        val description = doc.select("div.anime-description").text()
        val imageUrl = doc.select("div.anime-cover img").attr("src")
        val genres = doc.select("div.anime-genres span").map { it.text() }
        val rating = doc.select("div.anime-rating").text().toFloatOrNull() ?: 0f
    ...
    override suspend fun parseEpisodes(html: String): List<Episode> {
        ...
        return doc.select("div.episode-item").map { element ->
            val number = element.select("span.episode-number").text().toIntOrNull() ?: 0
            val title = element.select("span.episode-title").text()
    ...
    private fun parseAnimeCard(element: Element): Anime {
        val title = element.select("h3.anime-title").text()
